...
def salesforce_pull_persons(args: List[str]):
    print("_______ start func salesforce_pull_persons _______")
    try:
        arguments = parse_arguments(args)
        if "log_id" in arguments:
            handle_salesforce_pull_person(arguments["log_id"])
        else:
            db = Session(engine)
            salesforces = db.exec(
            ...
            for salesforce in salesforces:
                if salesforce.auto_pull_persons:
                    try:
                        salesforce_company_sync = SalesforceSyncHistories(
                            ...
                            type=TYPE_INTEGRATION_ENUM.PULL_PERSONS,
                            status=0,
                            method="AUTO",
                        ...
                        print(
                            "_______ error func salesforce_pull_persons (AUTO) _______",
                            str(e.detail),
                        )
                        continue
                else:
            ...
            for log_id in log_ids:
                try:
                    handle_salesforce_pull_person(log_id)
                except HTTPException as e:
                    print(
                        "_______ error log_id salesforce_pull_persons _______",
                        str(e.detail),
                    )
                    continue
    except HTTPException as e:
        print("_______ error func salesforce_pull_persons _______", str(e))
        raise e


def handle_salesforce_pull_person(log_id: str):
    ...
    try:
        ...
        if hasattr(e, "content"):
            ...
            error = str(e)
        print("_______ error of func handle_salesforce_pull_person _______", error)
