...
def salesforce_push_companies(args: List[str]):
    print(
        "_______ start func salesforce_push_companies _______",
    )
    try:
        arguments = parse_arguments(args)
        if "log_id" in arguments:
            handle_salesforce_push_companies(arguments["log_id"])
        else:
            # create new SyncHistory log_id and call handler
            pass
    except HTTPException as e:
        print("_______ error of func salesforce_push_companies _______", str(e))
        raise e


def handle_salesforce_push_companies(log_id: str):
    print("_______ start func handle_salesforce_push_companies _______")
    db = Session(engine)

    sync_history = db.exec(
        select(SalesforceSyncHistories).where(SalesforceSyncHistories.log_id == log_id)
    ).first()
    ...
    try:
        companies = db.exec(
            ...
            .join(
                ...
                and_(
                    ...
                    == SalesforceManualPushCompanies.ss_company_id,
                    SalesforceManualPushCompanies.deleted_at.is_(None),
                ),
            )
            .where(SalesforceManualPushCompanies.log_id == log_id)
        ).all()
        ...
        if hasattr(e, "content"):
            ...
            error = str(e)
        print("_______ error of func handle_salesforce_push_companies _______", error)
        ...
        print("_______ end func handle_salesforce_push_companies _______")
