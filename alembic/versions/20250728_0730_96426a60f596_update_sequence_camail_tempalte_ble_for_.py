"""update sequence camail tempalte ble for linkedin step

Revision ID: 96426a60f596
Revises: 8b8dee1f4868
Create Date: 2025-07-28 07:30:43.373794
...
revision = "96426a60f596"
down_revision = "8b8dee1f4868"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE sequence_mail_templates RENAME TO sequence_step_content_templates"
    )

    result = (
        op.get_bind()
        .execute(sa.text("SELECT 1 FROM pg_type WHERE typname = 'contenttype'"))
        .fetchone()
    )

    if not result:
        content_type_enum = sa.Enum(ContentType, name="contenttype")
        content_type_enum.create(op.get_bind())

    content_type_enum = sa.Enum(ContentType, name="contenttype")

    op.add_column(
        "sequence_step_content_templates",
        sa.Column("content_type", content_type_enum, nullable=True),
    )

    op.execute(
        f"""UPDATE sequence_step_content_templates SET content_type = '{ContentType.MAIL}' WHERE content_type IS NULL;"""
    )
...
def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    # Remove content_type column from the table
    op.drop_column("sequence_step_content_templates", "content_type")

    # Rename table back from sequence_step_content_templates to sequence_mail_templates
    op.execute(
        "ALTER TABLE sequence_step_content_templates RENAME TO sequence_mail_templates"
    )
    # ### end Alembic commands ###
