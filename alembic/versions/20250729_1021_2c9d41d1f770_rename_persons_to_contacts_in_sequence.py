"""rename persons to contacts in sequence

Revision ID: 2c9d41d1f770
Revises: 27fca2d1b292
Create Date: 2025-07-29 10:21:16.280396

"""
import sqlalchemy as sa

from alembic import op
from app.models.sequence.contact import ContactType
from app.models.sequence.mail_history import MAIL_HISTORY_VIEW_NAME

# revision identifiers, used by Alembic.
revision = "2c9d41d1f770"
down_revision = "3b299a6f1085"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.rename_table("sequence_persons", "sequence_contacts")
    op.rename_table("sequence_campaign_persons", "sequence_campaign_contacts")

    # Create enum type if not exists
    connection = op.get_bind()
    result = connection.execute(
        sa.text("SELECT 1 FROM pg_type WHERE typname = 'contacttype'")
    ).fetchone()

    if not result:
        contact_type_enum = sa.Enum(ContactType, name="contacttype")
        contact_type_enum.create(connection)

    contact_type_enum = sa.Enum(ContactType, name="contacttype")

    op.add_column(
        "sequence_contacts",
        sa.Column("target_type", contact_type_enum, nullable=True),
    )
    ...
    op.execute(
        f"""UPDATE sequence_contacts
        SET target_type = '{ContactType.PERSON}'
...
def downgrade() -> None:
    ...
    op.drop_column("sequence_contacts", "target_type")

    # Drop enum type
    op.execute("DROP TYPE IF EXISTS contacttype")

    op.rename_table("sequence_campaign_contacts", "sequence_campaign_persons")
    op.rename_table("sequence_contacts", "sequence_persons")
