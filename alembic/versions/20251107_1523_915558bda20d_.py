...
Revision ID: 915558bda20d
Revises: 93f046873877
Create Date: 2025-11-07 15:23:49.828383
...
revision = "915558bda20d"
down_revision = "93f046873877"
branch_labels = None
depends_on = None
        ...
        UNION
            ...
            (SELECT sm.user_id,
                    ...
                    row_number() OVER (PARTITION BY sm.team_id
                                       ORDER BY sm.is_default DESC) AS row_num
            FROM sequence_mailboxes sm WHERE sm.deleted_at IS NULL) tmp ON tmp.team_id = sc.team_id
            AND tmp.row_num = 1
            ...
            WHERE smh.id IS NULL
            AND scs.is_active = TRUE
            AND sc.is_active = TRUE
            AND scp.status = 'ACTIVE'
            AND scs.step_type = 'MAIL_AUTO'
            ...
            LEFT JOIN
                (
                    SELECT
                        ...
                        la.deleted_at,
                        row_number() OVER (
                            PARTITION BY la.team_id
                            ORDER BY la.is_default DESC
                        ) AS row_num
                    FROM
