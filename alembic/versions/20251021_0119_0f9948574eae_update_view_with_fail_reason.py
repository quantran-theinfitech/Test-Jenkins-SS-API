"""update_view_with_fail_reason

Revision ID: 0f9948574eae
Revises: d2bd5f5519f9
Create Date: 2025-10-21 01:19:30.097503
...
revision = "0f9948574eae"
down_revision = "d2bd5f5519f9"
branch_labels = None
depends_on = None


def upgrade() -> None:
    op.execute("ALTER TYPE mailhistorystatus ADD VALUE 'NOT_SENT';")
    ...
    CREATE OR REPLACE VIEW {MAIL_HISTORY_VIEW_NAME} AS (
        SELECT smh.id AS mail_history_id,
            ...
            smh.account_option,
            CASE
                WHEN smas.mailbox_alias_id IS NOT NULL
                    THEN sma.alias_email
                ELSE sm.email
            END AS email_from,
            smh.title,
            smh."content",
        ...
        LEFT JOIN sequence_mail_alias_settings smas ON smas.mailbox_id = smh.sequence_mailbox_id AND smas.sequence_campaign_id = smh.sequence_campaign_id AND smas.sequence_person_id = smh.sequence_contact_id
        LEFT JOIN sequence_mail_aliases sma ON smh.mailbox_alias_id = sma.id
        LEFT JOIN users u ON smh.sent_by = u.id
        WHERE smh.deleted_at IS NULL AND smh.to_address IS NOT NULL AND smh.to_address != ''
        UNION
            SELECT NULL as mail_history_id,
                ...
                tmp.account_option AS account_option,
                tmp.exact_email_from as email_from,
                tmp.mail_template_title AS title,
                tmp.mail_template_content AS content,
                tmp.to_email_address as to_address,
                'SCHEDULED' AS status,
                CASE
            ...
            (SELECT smh.*,
                    ...
                    sp.name AS sequence_person_name,
                    sp.email as to_email_address,
                    sc.name AS sequence_campaign_name,
                    scp.status AS sequence_person_status,
                    scs."order" AS sequence_step_order,
                    CASE
                        WHEN smas.mailbox_alias_id IS NULL
                            THEN sm.email
                        WHEN smas.mailbox_alias_id IS NOT NULL
                            THEN sma.alias_email
                    END AS exact_email_from,
            ...
            FROM sequence_mail_histories smh
                ...
                LEFT JOIN sequence_mailboxes sm ON smh.sequence_mailbox_id = sm.id
                LEFT JOIN sequence_mail_alias_settings smas ON smas.mailbox_id = smh.sequence_mailbox_id AND smas.sequence_campaign_id = smh.sequence_campaign_id AND smas.sequence_person_id = smh.sequence_contact_id
                LEFT JOIN sequence_mail_aliases sma ON smas.mailbox_alias_id = sma.id
                LEFT JOIN users u ON smh.sent_by = u.id
        ...
        UNION
            ...
            FROM sequence_mailboxes sm WHERE sm.deleted_at IS NULL) tmp ON tmp.user_id = sc.created_by
            AND tmp.row_num = 1
            ...
            FROM
                (
                    ...
                    LEFT JOIN users u ON
                        smh.sent_by = u.id
                    WHERE
                        scs.step_type in ('LINKEDIN_AUTO_MESSAGE'::steptype, 'LINKEDIN_CONNECTION_REQUEST'::steptype, 'LINKEDIN_VIEW_PROFILE'::steptype)
                        AND sp.linkedin_url IS NOT NULL
                        AND sp.linkedin_url != ''
                ) tmp
            LEFT JOIN sequence_mail_histories smh ON
...
def downgrade() -> None:
    op.execute(f"DROP VIEW IF EXISTS {MAIL_HISTORY_VIEW_NAME};")
    op.execute("UPDATE sequence_mail_histories SET status = 'FAILED' WHERE status = 'NOT_SENT';")
    op.execute("ALTER TYPE mailhistorystatus RENAME TO mailhistorystatus_old;")
    op.execute("CREATE TYPE mailhistorystatus AS ENUM ('SENT', 'FAILED', 'SKIPPED', 'SCHEDULED', 'OPENED', 'DRAFT', 'REPLIED', 'BOUNCED', 'OPT_OUT');")
    op.execute("ALTER TABLE sequence_mail_histories ALTER COLUMN status TYPE mailhistorystatus USING status::text::mailhistorystatus;")
    op.execute("DROP TYPE mailhistorystatus_old;")
    op.execute(
        f"""
    CREATE OR REPLACE VIEW {MAIL_HISTORY_VIEW_NAME} AS (
        SELECT smh.id AS mail_history_id,
