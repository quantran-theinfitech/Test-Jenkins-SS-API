...
def manual_sync_persons(db: Session, current_user: UserBase):
    ...
    hubspot_company_sync_detail = db.exec(
        select(HubspotCompanySyncHistories).where(
            ...
            HubspotCompanySyncHistories.type == TypeSync.IDENTIFY_PERSON,
    ...
    if hubspot_company_sync_detail:
        ...
        if hubspot_company_sync_detail.created_at and (
            ...
            raise TooManyRequestsException(
                detail="integration.hubspot.hasPullingPersons"
            )
    ...
    try:
        ...
        batch.submit_job_by_log_id(
            BatchManualAction.HUBSPOT_PULL_PERSONS, hubspot_company_sync.log_id
        )
