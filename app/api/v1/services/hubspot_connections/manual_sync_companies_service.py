...
def manual_sync_companies(db: Session, current_user: UserBase):
    ...
    hubspot_company_sync_detail = db.exec(
        select(HubspotCompanySyncHistories).where(
            ...
            HubspotCompanySyncHistories.type == TypeSync.PULL_FROM_HUBSPOT,
    ...
    try:
        hubspot_company_sync = HubspotCompanySyncHistories(
            ...
            type=TypeSync.PULL_FROM_HUBSPOT,
            status=0,
        ...
        batch.submit_job_by_log_id(
            BatchManualAction.HUBSPOT_PULL_COMPANIES, hubspot_company_sync.log_id
        )
