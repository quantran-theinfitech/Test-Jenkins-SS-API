...
from app.models.press_release_ingest import StateCode
...
def get_scenario_press_release_list(db: Session):
    query = """with last_subcriptions as (
        ...
        where t.listing_plan_code = :pre_plan_code
            ...
            and s.type_code = :press_release_scenario_code
            """
    listing_scenarios = db.execute(
        text(query),
        params={
            "pre_plan_code": PlanCode.UNLIMITED,
            "press_release_scenario_code": TypeCode.NEWS,
        },
    ).all()
...
def notify_new_press_release(db: Session):
    update_in_db_query = """
                UPDATE press_release_ingests
                SET state = :processing_state_code
                WHERE state = :in_db_state_code
                RETURNING press_release_ingest_id;
            """
    ...
    if len(ingest_ids) > 0:
        db.commit()
        listing_scenarios = get_scenario_press_release_list(db)
        listing_scenarios = [
            json.dumps(
        ...
        for c in chunks:
            if len(c) > 1:
                batch_client.submit_job(
                    ...
                    containerOverrides={
                        "command": ["notify-new-press-release", c],
                    },
                )
            else:
                batch_client.submit_job(
        ...
        update_processing_query = """
                    ...
                    WHERE state = :processing_state_code
                        AND press_release_ingest_id IN :ingest_ids
                """
        db.execute(
