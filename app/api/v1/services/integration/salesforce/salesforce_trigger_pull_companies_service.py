...
def salesforce_trigger_pull_companies(db: Session, current_user: UserBase):
    ...
    salesforce_sync_detail = db.exec(
        select(SalesforceSyncHistories).where(
            ...
            SalesforceSyncHistories.type == TYPE_INTEGRATION_ENUM.PULL_COMPANIES,
    ...
    try:
        salesforce_pull_companies = SalesforceSyncHistories(
        ...
        db.add(salesforce_pull_companies)
        db.flush()
        ...
        batch.submit_job_by_log_id(
            BatchManualAction.SALESFORCE_PULL_COMPANIES,
            salesforce_pull_companies.log_id,
        )

        return salesforce_pull_companies.id
